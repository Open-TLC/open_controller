...
class SharedSignalState:
    ...
    async def set_green(self, value: str):
        async with self._lock:
            self._green = bool(value)

    async def get_green(self) -> Optional[str]:
        async with self._lock:
            return self._green
...
async def signal_listener(nc: nats.NATS, signal_state: SharedSignalState):
    ...
    async def on_msg(msg):
        try:
            data = json.loads(msg.data.decode("utf-8"))
            sig_state = data.get("substate") 
            await signal_state.set_green(sig_state)
            print(f"[signal_listener] green={sig_state}")
        except Exception as e:
            print("[signal_listener] parse error:", e)
        ...
        try:
            ...
            if green: 
                if not(safety_ext_started):
                    if not(safety_ext): 
                        state = "Normal Extension Started"
                        ...
                        print ("Current state: ", state, "at: ", safety_ext_startet_at)
