...
async def processor(nc: nats.NATS, queue: asyncio.Queue, signal_state: SharedSignalState, threshold_m: float):
    ...
    while True:
        ...
        try:
            ...
            if green_started and (state == "Red"):
                state = "Green Started"
            ...
            if green: 
                if not(safety_ext_started):
                    if not(safety_ext) and (state == "Green Started"):
            ...
            green_ended = last_green and  not(green) and not(safety_ext)
            if green_ended and (state == "Safety Extension Mode"):
                state = "Green Ended"
