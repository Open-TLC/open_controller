...
class PhaseRingController:
    ...
    def __init__(self, conf: dict[str, Any], timer):
        ...
        for det in det_cnf:
            ...
            if (
                det_cnf[det]["type"] == "e3detector"
            ):  # DBIK240731 new detector type (e3) and extender types (e3)
                new_det = e3Detector(
                    self.timer, det, det_cnf[det]
                )  # create e3 detector
                # new_det.set_request_groups(self.groups) # DBIK 202511 Add req groups to e3dets
