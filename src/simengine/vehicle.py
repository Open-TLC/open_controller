...
class Vehicle:
    ...
    # waiting_rs_receipt->wait_rs_coupon is for getting the reputation from the Reputation Server (RS)
    ...
    states = ['waiting_for_data', 'data_to_send', 'waiting_ra_receipt', 'waiting_for_ra_certificate', 'waiting_rs_receipt', 'wait_rs_coupon']
    ...
    def __init__(self, id):
        ...
        self.machine.add_transition(trigger='request_rs_receipt', source='waiting_for_ra_certificate', dest='waiting_rs_receipt')
        # Then we send the measurement data to RS and expect to get back the coupon 
        # for the prooposed new reputation (step 7)
        self.machine.add_transition(trigger='send_data_to_rs', source='waiting_rs_receipt', dest='wait_rs_coupon')
    ...
    def process_ra_coupong(self, response):
        """
        Processes the certificate from the RA
        """
    ...
    def get_measurement_message(self):
        ...
        measurement_message = get_latest_data()
