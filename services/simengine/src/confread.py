...
if DIRECT:
    DEFAULT_SUBJECT_STAT_DETECTORS = (
        "detector.control"  # DBIK202509 Prefix for direct access to real controller
    )
else:
...
class GlobalConf:
    ...
    def set_vals_from_conf(self, filename):
        ...
        if "simulation" in config_from_file:
            ...
            if "nats" in config_from_file["simulation"]:
                self.conf["simulation"]["nats"].update(
                    config_from_file["simulation"]["nats"],
                )
        ...
        if "outputs" in config_from_file:
            if "det_outputs" in config_from_file["outputs"]:
                self.conf["outputs"]["det_outputs"].update(
                    config_from_file["outputs"]["det_outputs"],
                )
            if "sig_outputs" in config_from_file["outputs"]:
                self.conf["outputs"]["sig_outputs"].update(
                    config_from_file["outputs"]["sig_outputs"],
                )
        ...
        if "inputs" in config_from_file:
            if "sig_inputs" in config_from_file["inputs"]:
                self.conf["inputs"]["sig_inputs"] = config_from_file["inputs"][
                    "sig_inputs"
                ]
    ...
    def get_nats_params(self):
        ...
        return (
            str(self.conf["simulation"]["nats"]["ip"])
            + ":"
            + str(self.conf["simulation"]["nats"]["port"])
        )
    ...
    def get_radar_polygons(self):
        ...
        if not self.conf.get("outputs") or not self.conf["outputs"].get("rad_outputs"):
            return {}
        radars = {}
