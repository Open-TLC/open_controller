...
class SignalGroup(HierarchicalMachine):
    ...
    def __init__(
        ...
        self.controller_index = (
            controller_index  # groups assigned to controller are indexed from 1 upwards
        )
        ...
        self.fixed_amber = FixedTime(
            self.system_timer,
            self,
            min_length=grp_conf["min_amber"],
        )
        self.fixed_amber_red = FixedTime(
            self.system_timer,
            self,
            min_length=grp_conf["min_amber_red"],
        )
        # Should this be dependent on conf?
        self.va_green = VehicleActuated(
            self.system_timer,
            self,
        ...
        self.group_based_red = GroupBasedRed(
            self.system_timer,
            self,
            min_length=grp_conf["min_red"],
        )
        ...
        states = [
            ...
            {
                "name": "Red",
                "on_enter": self.start_red_cb,
                "children": self.group_based_red,
            },
            {"name": "AmberRed", "children": self.fixed_amber_red},
            {
                "name": "Green",
                "on_enter": self.start_green_cb,
                "children": self.va_green,
            },
            {
                "name": "Amber",
                "on_enter": self.start_amber_cb,
                "children": self.fixed_amber,
            },
        ]
        transitions = [
            {
        ...
        # Note: super() (not a hardcoded HierarchicalGraphMachine.__init__ call) so that this
    ...
    def start_red_cb(self):
        # Note: this is imitted if the unit is used as an web interface template, might
        # find a better way
    ...
    def group_in_conflict(self, group):
        """Return true if this group conflicts with a given group."""
        return any(c_group["group"] == group for c_group in self.conflicting_groups)
    ...
    def __str__(self):
        ...
        ret += f"  Delaying: {self.delaying_groups}"  # DBIK231209
        return ret

    def get_grp_state(self):
        """Return status (char) of the group in 'traditional' format."""
        grp_statuses = {
            "Red_MinimumTime": "a",
            "Red_CanEnd": "b",
            "Red_ForceGreen": "f",
    ...
    def get_sumo_state(self):
        """Return status in sumo format (char)."""
    ...
    def get_params(self):
        ...
        if "delaying_groups" in params:
            # FIXME: Modifying objects internal state in a getter.
            # Should create a deep copy of self.grp_conf before modifying it.
        ...
        for grp in self.conflicting_groups:
            ...
            if grp["group"].state in ["Green_RemainGreen"]:
                if not (self.any_nonconflicting_green_active(grp)):
                    grp[
                        "group"
                    ].other_group_requests_end_green = True  # Set the request
                else:
                    grp[
                        "group"
                    ].other_group_requests_end_green = False  # Don't set the request

        for grp in self.conflicting_groups:
            if grp["group"].state in [
                "Red_ForceGreen",
                "Red_WaitIntergreen",
                "AmberRed_MinimumTime",
                "Green_MinimumTime",
    ...
    def has_green_request(self):
        ...
        """
        if (
            self.grp_conf["request_type"] == "fixed"
        ):  # DBIK231217 Fixed-mode has to set the request, not return True
    ...
    def intergreens_passed(self):
        ...
        for grp in self.conflicting_groups:
            if grp["group"].group_on():
                ...
                return False
            if (
                grp["delay"] + grp["group"].amber_started_at > self.system_timer.seconds
            ):  # DBIK20231023 Intergreen counting start from beginnig of amber
    ...
    def start_delay_not_passed(self, grp_conf):
        """Returns true if start delay of pedestrian signals have not passed"""
        if "delaying_groups" not in grp_conf:
            return False
        ...
        if self.delaying_groups:
            for dgrp in self.delaying_groups:
                ...
                if self.system_timer.seconds < (dgrp.green_started_at + startdelay):
                    time_left = round(
                        (dgrp.green_started_at + startdelay)
                        - self.system_timer.seconds,
                        1,
                    )
                    print(
                        "Wait group: ",
                        ...
                        dgrp.group_name,
                        "Delay started: ",
                        dgrp.green_started_at,
                        "Start delay: ",
                        startdelay,
                        "Delay left: ",
                        time_left,
                    )  # DBIK20260326 Debugged
                    return True  # Start delay not passed
        return False
    ...
    def group_on(self):
        ...
        """
        par_st = self.state.split("_")[0]
        return par_st in ("Green", "Amber", "AmberRed")
    ...
    def is_in_phase_min_time(self):
        if self.state in ["Red_WaitIntergreen", "AmberRed_MinimumTime"] or (
            (
                self.state
                in ["Green_MinimumTime", "Green_Extending", "Green_RemainGreen"]
            )
            and (self.system_timer.seconds - self.green_started_at < 3.0)
        ):
            return True
        return False
    ...
    def phase_min_time_reached2(self):
        ...
        if (
            self.state in ["Green_MinimumTime", "Green_Extending", "Green_RemainGreen"]
        ) and (phasetime > 3.0):
            return phasetime
        return 0.0
    ...
    def min_green_start(self):
        ...
        if self.prev_state in ["Any"] and self.state in [
            "Green_MinimumTime",
        ]:  # DBIK241107
            return True
        return False
    ...
    def conflicting_active_green_passed(self):
        ...
        for grp in self.conflicting_groups:
            if grp["group"].state not in [
                "Green_RemainGreen",
                "Amber_MinimumTime",
                "Red_MinimumTime",
                "Red_CanEnd",
            ]:
                active_conf_greens_passed = (
                    False  # Found one conflicting active green signal
                )
...
class FixedTime(HierarchicalMachine):
    ...
    def min_time_passed(self):
        ...
        if self.system_timer:
            return self.min_started_at + self.min_length < self.system_timer.seconds
        return False
...
class VehicleActuated(FixedTime):
    ...
    def __init__(self, system_timer, group, min_length, max_length, green_end):
        ...
        self.add_transition(
            ...
            dest="RemainGreen",
            conditions=[
                self.other_group_request_priority,
                self.remain_green_mode,
            ],  # DBIK241029 add new transition condition for priority request
        )
        ...
        self.add_transition(
            ...
            dest="Extending",
            conditions=[
                self.external_extending,
                self.extension_repetitive,
            ],  # DBIK20231018 We should add one shot mode extension mode
        )
    ...
    def other_group_request_priority(self):
        return self.group.other_request_level > self.group.own_request_level
...
class GroupBasedRed(FixedTime):
    ...
    def __init__(self, system_timer, group, min_length):
        super().__init__(system_timer, group, min_length)
        self.system_timer = system_timer
        self.group = group
        ...
        self.add_transition(
            ...
            dest="ForceGreen",
            conditions=[
                self.group.has_green_request,
                self.group.has_green_permission,
            ],  # self.group.can_conflicting_greens_terminated  DBIK230920 Next phase started too early, previous phase not fully served
    ...
    def can_start_cb(self):
        self.other_group_requests_end_green = (
            False  # DBIK 230913 Reset requests from other groups
        )
        if self.group.instant_transfer:
            self.group.next_state()
...
def _build_signal_group():
    ...
    return SignalGroup(
        timer,
        group_name,
        sys_cnf["controller"]["signal_groups"][group_name],
    )
