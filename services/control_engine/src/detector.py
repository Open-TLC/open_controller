...
def main():
    print("testing dets")
    conf = {"sumo_id": "0", "request_groups": ["group0", "group1"]}
    ...
    def loop_on(self, set_on):
        if set_on and not self._loop_on:
            ...
            for grp in self.request_groups:
                ...
                grp.own_request_level = (
                    self.priolevel
                )  # DBIK241029 pass the request priority level to the signal group
                for confgrp in grp.conflicting_groups:  # DBIK241101 pass the request priority level to the conflicting signal groups
                    ...
                    if self.priolevel > other_req_level:
                        confgrp[
                            ...
                            self.priolevel
                        )  # DBIK201105 Set only if higher request level
...
class ExtDetector(Detector):
    ...
    def is_extending(self):
        ...
        return self.loop_on or (
            (self.detection_end_at + self.ext_time) > self.system_timer.seconds
        )
...
class GrpDetector(Detector):
    ...
    def is_extending(self):
        ...
        if self.extgroup.green_started_at < 0:
            return False
        ext_time_on = (
            self.extgroup.green_started_at + self.ext_time
...
class e3Detector(Detector):
    ...
    def update_e3_vehicles(self, obj_list):
        ...
        for vehid in self.det_vehicles_dict:
            ...
            TLSdist = self.det_vehicles_dict[vehid].get("TLSdist", "not_found")

            TLSno = self.det_vehicles_dict[vehid].get(
                "TLSno", "not_found"
            )  # DBIK202602 Set traffic signal number

            if TLSdist != "not_found":
                if TLSdist > self.MaxDist:
                    continue

            if (self.vtypes != []) and vtype not in self.vtypes:
                BP = 1
                continue
            ...
            if vtype == "car_type":
                self.vehcount += (
                    1  # DBIK202602 Give the basic weight for passenger cars
                )
            elif vtype == "truck_type":
                self.vehcount += (
                    3  # DBIK240923 Add extra weight for trucks 1 truck = 3 vehs
                )
            elif vtype == "tram_type":
                self.vehcount += (
                    100  # DBIK240923 Add extra weight for trams 1 tram = 100 vehs
                )
            ...
            # Special setting for JS270T, should be configured in init-file DBIK20241025
                ...
                if self.v2x_ON:
                    self.det_vehicles_dict[vehid]["vcolor"] = (
                        "blue"  # V2X vehicle detected
                    )
                    if (
                        (TLSno == 1)
                        and (TLSdist < self.MaxOZ)
                        and (TLSdist > self.MinOZ)
                    ):
                        self.det_vehicles_dict[vehid]["vcolor"] = (
                            "green"  # V2X veh at option-zone
                        )

                        leaderDist = self.det_vehicles_dict[vehid]["leaderDist"]
                        if (leaderDist < self.SafeDist) and (leaderDist > 0):
                            self.det_vehicles_dict[vehid]["vcolor"] = (
                                "yellow"  # V2X veh too close to vehicle in front
                            )
                            self.ShortGapFound = True

                            if self.SafeExtOn:
                                self.det_vehicles_dict[vehid]["vcolor"] = (
                                    "red"  # Safety extension ON for V2X veh
                                )
                                curspeed = self.det_vehicles_dict[vehid]["speed"]

                                leaderSpeed = self.det_vehicles_dict[vehid][
                                    "leaderSpeed"
                                ]
                                # VX2newSpeed = leaderSpeed - 5.0
                                VX2newSpeed = 10.0
                                if VX2newSpeed > 4.0:
                                    self.det_vehicles_dict[vehid]["set_speed"] = (
                                        VX2newSpeed  # V2X vehicle slow down
                                    )
